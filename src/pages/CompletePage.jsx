...
export function CompletePage() {
  ...
  const { lang } = useLang();
  const T = translations[lang].complete;
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ backgroundColor: '#0F0F14' }}>
      <header className="px-5 py-5" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        <Link to="/" className="font-display text-lg font-bold" style={{ color: '#F0F0F5' }}>Music for U</Link>
      </header>
      ...
      <main className="flex-1 max-w-lg mx-auto w-full px-5 py-10">
        <div className="text-center mb-10 animate-fade-in-up">
          <div className="inline-flex items-center justify-center w-20 h-20 rounded-full mb-6 text-4xl"
            style={{ backgroundColor: 'rgba(107,163,214,0.12)' }}>🎁</div>
          <h1 className="text-2xl font-bold mb-3 whitespace-pre-line" style={{ color: '#F0F0F5' }}>{T.title}</h1>
          <p className="text-sm leading-relaxed" style={{ color: '#9090A8' }}>
            {lang === 'en'
              ? <>{receiverName}{T.sub1}<br />{T.sub2} <strong style={{ color: '#F0F0F5' }}>{email}</strong>{T.sub3}</>
              : lang === 'ja'
              ? <>{receiverName}{T.sub1}<br />{T.sub2}<strong style={{ color: '#F0F0F5' }}>{email}</strong>{T.sub3}</>
              : <>{receiverName}{T.sub1}<br />{T.sub2} <strong style={{ color: '#F0F0F5' }}>{email}</strong>{T.sub3}</>
            }
        ...
        <div className="animate-fade-in-up delay-400">
          <Link to="/"><Button fullWidth variant="secondary" size="lg">{T.backHome}</Button></Link>
        </div>
      </main>
    </div>
  );
